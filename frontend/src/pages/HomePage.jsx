  ...
  return (
    <>
      ...
      <section className="relative h-[100dvh] w-full max-w-[100vw] overflow-hidden bg-gray-900">
        {slides.length > 0 ? (
          <>
            {slides.map((slide, i) => (
              <div key={i} className={`absolute inset-0 overflow-hidden transition-opacity duration-1000 ease-in-out ${i === current ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
                <img src={slide.image} alt={slide.title} loading={i === current ? 'eager' : 'lazy'} className={`w-full h-full object-cover transition-transform duration-300 ease-out ${i === current ? 'scale-102' : 'scale-100'} will-change-transform`} onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/1920x1080/1a1a2e/4338ca?text=Shiv+Events'; }} />
                <div className="absolute inset-0 bg-gradient-to-b from-black/50 via-black/20 to-black/70" />
              </div>
            ))}
            <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-4">
              <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold text-white mb-6 drop-shadow-xl tracking-wide" style={{ fontFamily: "'Cinzel', serif", textShadow: '0 4px 20px rgba(0,0,0,0.8)' }}>
                {slides[current]?.title}
              </h1>
              <p className="text-white/90 text-xl md:text-2xl mb-10 max-w-3xl font-light" style={{ fontFamily: "'Playfair Display', serif" }}>{slides[current]?.subtitle}</p>
              <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-4">
                <Link to="/booking" className="btn-primary text-base px-8 py-4 shadow-lg">🎉 Book Your Event</Link>
                <a href="https://wa.me/916394352002" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#25D366] text-white font-medium rounded-xl hover:bg-[#128C7E] transition-all shadow-lg">📱 +91 63943 52002</a>
          ...
          <div className="relative h-full bg-gradient-to-br from-gray-900 via-primary-900 to-purple-900 flex flex-col items-center justify-center text-center px-4 animate-blur-in overflow-hidden">
            ...
            <h1 className="relative z-10 text-5xl md:text-7xl lg:text-8xl font-extrabold mb-6 tracking-wide animate-fadeInDown drop-shadow-2xl text-transparent bg-clip-text bg-gradient-to-r from-white via-primary-200 to-white font-cinzel">
              {settings?.companyName || 'Shiv Event Management'}
            </h1>
            <p className="relative z-10 text-white/90 text-xl md:text-3xl mb-12 font-light tracking-wide animate-fadeInUp font-playfair">{settings?.tagline || 'Premium Event Management'}</p>
        ...
        <section className="relative py-20 overflow-hidden animate-blur-in">
          ...
          <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 relative z-10">
            {stats.eventsCompleted && (
              <div className="card p-6 md:p-8 bg-white/10 backdrop-blur-md border border-white/20 text-center hover:-translate-y-1 hover:bg-white/20 transition-all duration-200 group animate-fadeInUp">
      ...
      <section className="py-24 relative overflow-hidden bg-gradient-to-br from-indigo-900 via-purple-900 to-fuchsia-900 animate-fadeInUp">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <div className="text-center mb-16">
            ...
            <h2 className="text-5xl md:text-7xl font-bold text-white mb-6 animate-fadeInUp drop-shadow-2xl font-cinzel" style={{ animationDelay: '0.1s' }}>Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-300 to-yellow-300">Services</span></h2>
            <p className="text-indigo-100 text-xl md:text-2xl max-w-3xl mx-auto animate-fadeInUp font-light drop-shadow-md font-playfair" style={{ animationDelay: '0.2s' }}>From intimate gatherings to grand spectacles, we handle every detail with precision, creativity, and a splash of magic.</p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {services.map((s, idx) => (
                <Link key={s._id} to={`/services/${s.slug}`}
                  className="group relative bg-white/10 backdrop-blur-md rounded-3xl shadow-lg hover:shadow-xl hover:shadow-pink-500/20 transition-all duration-200 hover:-translate-y-1 animate-fadeInUp flex flex-col overflow-hidden border border-white/20 hover:border-pink-300/50 will-change-transform"
                  ...
                  {s.image ? (
                    <div className="h-52 overflow-hidden relative shrink-0 z-10">
                      ...
                      <img src={s.image} alt={s.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105 lazy" loading="lazy" onError={(e) => { e.target.style.display = 'none'; }} />
                  ...
                  <div className="p-8 flex flex-col flex-1 relative z-10">
                    <h3 className="text-3xl font-bold text-white mb-3 group-hover:text-pink-300 transition-colors drop-shadow-md font-cinzel">{s.title}</h3>
                    <p className="text-indigo-100/90 text-lg leading-relaxed flex-1 font-playfair">{s.shortDescription || s.description?.slice(0, 100)}</p>
          ...
          <div className="max-w-7xl mx-auto px-4 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <div className="animate-fadeInLeft">
                ...
                <h2 className="text-5xl md:text-7xl font-bold text-white mb-6 drop-shadow-2xl leading-tight font-cinzel">Events That <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-100">Exceed</span> Expectations</h2>
                <p className="text-white/90 leading-relaxed mb-10 text-xl md:text-2xl animate-fadeInUp drop-shadow-md font-light font-playfair">{settings.aboutText}</p>
                <Link to="/booking" className="bg-white text-rose-600 font-bold px-10 py-5 rounded-xl shadow-[0_0_30px_rgba(255,255,255,0.3)] hover:shadow-[0_0_50px_rgba(255,255,255,0.6)] hover:-translate-y-2 transition-all duration-300 mt-2 inline-flex animate-fadeInUp text-lg tracking-wide font-cinzel" style={{ animationDelay: '0.2s' }}>✨ Get a Free Quote</Link>
              </div>
              <div className="grid grid-cols-2 gap-4 animate-fadeInRight">
                <div className="rounded-3xl h-48 w-full bg-white/20 backdrop-blur-sm border border-white/40 shadow-lg flex items-center justify-center hover:scale-105 hover:bg-white/30 transition-all duration-200 group will-change-transform">
                  <span className="text-7xl animate-float drop-shadow-lg group-hover:rotate-6 transition-transform duration-300">🎉</span>
                </div>
        ...
        <section className="py-24 px-4 bg-gradient-to-br from-teal-600 via-emerald-600 to-cyan-700 animate-fadeInUp relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto relative z-10">
            <div className="text-center mb-16">
              ...
              <h2 className="text-5xl md:text-7xl font-bold text-white mb-6 drop-shadow-2xl font-cinzel">What Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-green-200">Clients</span> Say</h2>
              <p className="text-teal-50 text-xl md:text-2xl max-w-3xl mx-auto animate-fadeInUp drop-shadow-md font-light font-playfair" style={{ animationDelay: '0.1s' }}>Don't just take our word for it. Read the stories of unforgettable moments we've created for people just like you.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {testimonials.map((t, idx) => (
                <div key={t._id} className="p-8 bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl hover:border-white/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-200 animate-fadeInUp flex flex-col relative group will-change-transform" style={{ animationDelay: `${idx * 0.08}s` }}>
                  ...
                  <p className="text-white/90 leading-relaxed mb-8 text-xl italic flex-1 relative z-10 font-medium font-playfair">
                    "{t.text}"
                  </p>
                  <div className="flex items-center gap-4 relative z-10 pt-6 border-t border-white/20">
                    ...
                    <div>
                      <div className="text-white font-bold text-xl drop-shadow-md group-hover:text-yellow-200 transition-colors font-cinzel">{t.name}</div>
      ...
      <section className="py-24 px-4 animate-fadeInUp bg-gray-900 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10 bg-gradient-to-br from-violet-600 via-fuchsia-600 to-pink-600 rounded-[3rem] p-12 md:p-24 shadow-[0_0_30px_rgba(217,70,239,0.3)] hover:shadow-[0_0_50px_rgba(217,70,239,0.5)] transition-all duration-300 overflow-hidden group border border-white/20 will-change-transform">
          ...
          <h2 className="relative z-10 text-5xl md:text-6xl lg:text-7xl font-extrabold text-white mb-6 animate-fadeInDown drop-shadow-2xl leading-tight font-cinzel">Ready to Plan Your Dream Event?</h2>
          <p className="relative z-10 text-pink-100 text-xl md:text-3xl mb-12 max-w-3xl mx-auto animate-fadeInUp font-light drop-shadow-lg font-playfair">Let's create something unforgettable together. Get a free consultation today and bring your vision to life.</p>
          <div className="relative z-10 flex flex-col sm:flex-row flex-wrap gap-6 justify-center">
            <Link to="/booking" className="inline-flex items-center justify-center gap-3 px-10 py-5 bg-white text-fuchsia-700 font-black rounded-2xl shadow-2xl hover:shadow-[0_0_40px_rgba(255,255,255,0.8)] transition-all text-lg hover:-translate-y-2 duration-300 animate-slideInUp tracking-wide font-cinzel">🎉 Book Now</Link>
            <a href="https://wa.me/916394352002" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-3 px-10 py-5 bg-[#25D366] text-white font-black rounded-2xl shadow-2xl hover:shadow-[0_0_40px_rgba(37,211,102,0.8)] transition-all text-lg hover:-translate-y-2 duration-300 animate-slideInUp tracking-wide font-cinzel" style={{ animationDelay: '0.05s' }}>📱 +91 63943 52002</a>
            <Link to="/contact" className="inline-flex items-center justify-center gap-3 px-10 py-5 bg-white/20 backdrop-blur-xl border border-white/40 text-white font-black rounded-2xl hover:bg-white/30 hover:shadow-[0_0_40px_rgba(255,255,255,0.4)] hover:-translate-y-2 transition-all text-lg duration-300 animate-slideInUp tracking-wide font-cinzel" style={{ animationDelay: '0.1s' }}>💬 Contact Us</Link>
          </div>
        </div>
      </section>
